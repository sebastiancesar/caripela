...
export class PlayPage {
  ...
  start() {
    if (!this.isPlaying) {
      ...
      this.points = 0;
      this.pointsState = EMPTY_POINTS;
      this.globalPoints = 0;
      this.faceLabelState = 'empty';
      this.heightBar = '0%';
      this.countdownApi.next({
        name: 'reset'
      });
    }
  ...
  /**
   * Update the interface based on the given @prediction.
   * 
   * @param prediction - A prediction which tells how confident is the model about
   * this classification for this category. 
   */
  ...
  predictMock() {
    this.predicted({
      class_id: this.faceToGet.class_id,
      confidence: 99
    });
  }
  startMock() {
    ...
    let obs = Observable
      ...
      .map(x => {
        return {
          class_id: this.getRandom(0, 4),
          confidence: (this.getRandom(80, 100) / 100).toFixed(2)
        };
      });
    this.startedStream = obs.subscribe(this.predicted.bind(this));
  }
